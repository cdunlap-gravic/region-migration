                ...
                if default_value is not None:
                    if value_type == int:
                        ...
                        if isinstance(default_value, tuple):
                            ...
                            # Handle string 'X,Y,Z' default values when expected type is tuple
...
def main():
    ...
    settings = {
        ...
        'source_bounding_boxes': [], # Bounding boxes are not saved as defaults in settings.json
    ...
    # Note: These are now directly populated from the 'settings' dict which was initialized from loaded_defaults
    # No need for separate initial_creative_mode_default etc. variables anymore.
    ...
    while not input_phase_complete:
        ...
        # overall_src_max_coords = (0,0,0) # Not strictly needed for paste calculation, but useful for info
        ...
        if settings['source_bounding_boxes']:
            ...
            # overall_max_x = settings['source_bounding_boxes'][0][3]
            # overall_max_y = settings['source_bounding_boxes'][0][4]
            # overall_max_z = settings['source_bounding_boxes'][0][5]

            # Iterate through the rest to find true overall min/max
            for bbox in settings['source_bounding_boxes']: # Changed from [1:] to iterate all, first bbox handles init
                overall_min_x = min(overall_min_x, bbox[0])
                overall_min_y = min(overall_min_y, bbox[1])
                overall_min_z = min(overall_min_z, bbox[2])
                # overall_max_x = max(overall_max_x, bbox[3])
                # overall_max_y = max(overall_max_y, bbox[4])
                # overall_max_z = max(overall_max_z, bbox[5])
        ...
        print(f"Overall Source Min Coords (Anchor): ({overall_src_min_coords[0]}, {overall_src_min_coords[1]}, {overall_src_min_coords[2]})")
    ...
    # No changes needed here, all_sub_regions is already populated correctly above
